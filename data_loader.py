...
class bahamasXLDMOData:
    ...
    def __init__(self, test_cosmology=False, bins=100, cutoff=(-1, 10), cosmo_params=np.arange(9), pk='nbk-rebin-std', lin='class', boost=True, holdout=False, sigma8=False, plot_hypercube=False, weight_k=True):
        ...
        if pk == 'powmes' and weight_k == True:
            correction_factor = [1.2761424, 1.1154015, 1.0447197, 1.0151449, 1.0195166, 1.0112120]
        ...
        if isinstance(holdout, bool):
            ...
            if test_cosmology != False:
                self.X_test = (self.test_cosmology-self.design_min)/(self.design_max-self.design_min)
        else:
            self.X_test = self.parameters_norm[self.holdout, :]
            self.X_train = np.delete(self.parameters_norm, self.holdout, axis=0)
        ...
        if pk == 'nbk-rebin-std':
            ...
            if pk == 'nbk-rebin':
                ...
                if weight_k == True:
                    random_model[:6, 0] = random_model[:6, 0]*correction_factor
                    powmes_low_res_model[:6] = powmes_low_res_model[:6]*correction_factor
        ...
        if pk == 'powmes':
            ...
            for index, directory in enumerate(training_directory):
                ...
                if weight_k == True:
                    self.k[index, :6] = self.k[index, :6]*correction_factor
        ...
        if isinstance(holdout, bool):
            ...
            self.Y_test = self.P_k_nonlinear[self.holdout, :]
            self.Y_train = np.log10(np.delete(self.P_k_nonlinear, self.holdout, axis=0))
    ...
    def plot_k(self, file_type='png'):
        ...
        #linear_med_limit=linear_low_limit=linear_high_limit=self.array_size+1
        linear_limit = []
...
if __name__ == "__main__":
    ...
    nbk_boost = bahamasXLDMOData(pk='powmes', lin='class', holdout=test_models)
