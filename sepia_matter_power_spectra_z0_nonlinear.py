...
def emulator_nonlinear(bins, test_model, pc, samp, step, mcmc, mcmc_params = False, plots = False, save = False, boost = True, cosmo = np.arange(9), lin = 'class', pk = 'reg'):
    ...
    #if high_k_cut > max(model_000_k) or high_k_cut > max(model_050_k) or high_k_cut > max(model_100_k):
    #    print('000 = ' + str(max(model_000_k)))
    #    print('050 = ' + str(max(model_050_k)))
    #    print('100 = ' + str(max(model_100_k)))
    #    high_k_cut = min([max(model_000_k), max(model_050_k), max(model_000_k)])
    #    print(high_k_cut)

    #if low_k_cut < min(model_000_k) or low_k_cut < min(model_050_k) or low_k_cut < min(model_100_k):
    #    print('000 = ' + str(min(model_000_k)))
    #    print('050 = ' + str(min(model_050_k)))
    #    print('100 = ' + str(min(model_100_k)))
    #    low_k_cut = max([min(model_000_k), min(model_050_k), min(model_100_k)])
    #    print(low_k_cut)
    ...
    for i in range(0, 150):
        ...
        if low_lim == -1:
            low_lim = 0
        if upp_lim >= len(model_000_k) or upp_lim >= len(model_050_k) or upp_lim >= len(model_100_k):
            x = -1
        print(x)
        ...
        if pk == 'reg':
            ...
            k_z0 = np.loadtxt('/storage/users/arijsalc/BAHAMAS_XL/DMO/model_' + f"{i:03d}" + '_N1260_L' + f"{size}" + '_DMO/PS/PS_k_078.csv', skiprows = 1, usecols = 0)*(parameters_z0[index, 2])

            P_k_z0 = np.loadtxt('/storage/users/arijsalc/BAHAMAS_XL/DMO/model_' + f"{i:03d}" + '_N1260_L' + f"{size}" + '_DMO/PS/PS_k_078.csv', skiprows = 1, usecols = 1)/(parameters_z0[index, 2]**3)

            h = interpolate.interp1d(k_z0, P_k_z0, kind='cubic', fill_value="extrapolate")
        ...
        if lin == 'camb':
            k_z0_linear = np.logspace(-3, np.log10(50), 300)*(parameters_z0[index, 2])
            P_k_camb_linear = np.loadtxt('/home/arijconl/BAHAMAS_XL/pk_lin_camb2022_slhs_nested_3x50_kmax_50_running_w0_m1p2_m0p6_wa_m1p6_p0p5.txt', skiprows=index, max_rows=1)/(parameters_z0[index, 2]**3)
        ...
        if boost == True:
            P_k_z0_nonlinear[index, :(upp_lim-low_lim+x)] = P_k_z0_interp[index, :(upp_lim-low_lim+x)] / y
        elif boost == False:
            P_k_z0_nonlinear[index, :(upp_lim-low_lim+x)] = P_k_z0_interp[index, :(upp_lim-low_lim+x)]
...
if __name__ == "__main__":
    test_k, error, test_start, test_end = emulator_nonlinear(bins, test_model, pc, samp, step, mcmc, mcmc_params, plots, pk = 'nbk')
